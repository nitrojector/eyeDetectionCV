...
def standardize(img, close):
    img = cv.resize(img,(256,256))
    gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    ret, binImg = cv.threshold(gray, lowAbsThre, highAbsThre, cv.THRESH_BINARY)
    if close:
...
def computePercent(suffix):
    global investigateMC, investigateCO
    print(suffix)
    if(suffix == 'L'):
        gazeDirection = 0
    elif(suffix == 'M'):
    ...
    for i in range(totalRecs):
        ...
        img0 = standardize(samp0, testClose)
        img1 = standardize(samp1, testClose)
    ...
    print("Test complete in folder " + str(inputFolder) + " with " + str(totalRecs) +
          " instances | lowBinThresh = " + str(lowAbsThre) + " | highBinThresh = " +
          str(highAbsThre) + " | testClose = " + str(testClose))
